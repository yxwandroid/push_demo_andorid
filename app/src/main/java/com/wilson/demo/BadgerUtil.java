...
public class BadgerUtil {
    ...
    private static Notification getNotification2(Context context, int badgeCount,String title,String msg) {
        if (notificationManager == null) {
            notificationManager = ((NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE));
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                //8.0之后添加角标需要NotificationChannel
     ...
     * 解决方案是清理掉之前发送的通知，并在app退到后台的时候执行添加角标操作
     */
    private static void badgeXiaomi2(final Context context, final int badgeCount, final Notification notification) {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                //延迟1秒是为了避免执行操作的时候还在app内，如要真正避免还是需要控制调用的时机
